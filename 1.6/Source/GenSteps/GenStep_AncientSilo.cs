...
namespace VanillaQuestsExpandedDeadlife
{
    public class GenStep_AncientSilo : GenStep
    {
        ...
        {
           Pawn pawn = PawnGenerator.GeneratePawn(DefDatabase<PawnKindDef>.AllDefsListForReading.Where(x => x.race.race.Animal == true && x.race.race.baseBodySize <= 2f && !StaticCollectionsClass.questDisabledAnimals.Contains(x)).RandomElement(), Faction.OfEntities);
